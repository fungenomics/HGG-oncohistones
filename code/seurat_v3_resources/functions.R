# Functions used for the seurat_v3 workflow in the single cell pipeline
# Selin Jessa and Marie Coutelier
...
compute_cell_cycle_whitfield <- function(seurat,
  ...
  g1.s.genes <- dplyr::filter(cell.cycle.genes, phase == "G1/S") %>% .$gene.symbol
  g2.m.genes <- dplyr::filter(cell.cycle.genes, phase == "G2/M") %>% .$gene.symbol
