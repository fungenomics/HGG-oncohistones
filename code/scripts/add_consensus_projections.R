...
seurat_meta <- seurat@meta.data %>%
    ...
    dplyr::select(rownames, cellname, sample.id, Malignant_normal_consensus, Cell_type_mouse_correlations = Type) %>%
    left_join(meta_sc %>% dplyr::select(sample.id = scRNAseq_sample, ID_paper)) %>% 
