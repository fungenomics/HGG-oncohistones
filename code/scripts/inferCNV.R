...
# and gene annotation. This script also produces a custom heatmap where the cells are
# labeled by the transcriptional cluster, # UMIs, and expression of some
# genes which mark common normal cell types.
#
# Usage:
# This should be run in the `scRNAseq/data/pipeline/10X/<sample>/inferCNV` folder. It 
# can be launched by a slurm submission script placed inside that folder
# (Example: `final/code/scripts/run_inferCNV.sh`).
# It expects the following config file to be present:
#
...
#   must be supplied, with the defaults below if desired. Example: `final/code/infos_templates/inferCNV.info.experiment.tsv`
#
...
info_experiment_tsv %>% add_case(param = "git_last_commit",
					   value = git2r::last_commit(repo = "/lustre06/project/6004736/sjessa/from_narval/HGG-oncohistones/stable")$sha %>% stringr::str_sub(1, 7)) %>%
...
#' 
#' inferCNV generates a heatmap by default when running infercnv::run(), this function
#' generates a second heatmap with additional customizations and covariates plotted.
...
plot_profile <- function(out,
  ...
  # and the counts are added here in a loop. Rationale for choosing these genes is that
  # they're expressed in possible normal cell populations:
  # DOCK8 (immune), VWF (endothelial), KCNJ8 (pericyes), MBP (myelinating oligodendrocytes)
